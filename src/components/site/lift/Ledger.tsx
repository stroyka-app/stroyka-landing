...
export default function Ledger({
  ...
  return (
    <div className="absolute right-6 top-1/2 w-[340px] -translate-y-1/2 lg:right-10 xl:w-[370px]">
      <div className="relative rounded-[22px] bg-site-night/80 p-6 text-site-paper shadow-[0_30px_80px_-30px_rgba(0,0,0,0.6)] ring-1 ring-site-paper/10 backdrop-blur-xl">
        ...
        <ul className="mt-5 space-y-0 border-t border-site-paper/10">
          {LOADS.map((load, i) => {
            const on = i < landed;
            return (
              <li key={load.id} className="relative flex h-[42px] items-center justify-between border-b border-site-paper/[0.07] text-[13.5px]">
                <AnimatePresence initial={false}>
                  {on ? (
                    <motion.div
                      key="on"
                      className="flex w-full items-center justify-between"
                      initial={reduced ? false : { opacity: 0, x: -14, filter: "blur(4px)" }}
